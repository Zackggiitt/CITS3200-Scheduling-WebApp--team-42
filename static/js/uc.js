  ...
  if (autoAssignBtn) {
    autoAssignBtn.addEventListener('click', async () => {
      try {
        ...
        const unitId = getUnitId();
        if (!unitId) {
          alert('No unit selected. Please select a unit first.');
          return;
        }
        
        // Call the auto assign endpoint
        const response = await fetch(`/unitcoordinator/units/${unitId}/auto_assign`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': CSRF_TOKEN
        ...
        if (result.ok) {
          ...
          let message = `Auto assignment completed!\n\n`;
          message += `Total sessions: ${result.total_sessions}\n`;
          message += `Assigned sessions: ${result.assigned_sessions}\n`;
          message += `Assignment rate: ${result.assignment_rate}%\n\n`;
          
          if (result.assignments && result.assignments.length > 0) {
            message += `Successful assignments:\n`;
            result.assignments.slice(0, 5).forEach(assignment => {
              message += `• ${assignment.session_info} → ${assignment.facilitator_name}\n`;
            });
            if (result.assignments.length > 5) {
              message += `... and ${result.assignments.length - 5} more\n`;
            }
          }
          ...
          if (result.conflicts && result.conflicts.length > 0) {
            ...
            result.conflicts.slice(0, 3).forEach(conflict => {
              message += `• ${conflict.session_info}: ${conflict.reason}\n`;
            });
            if (result.conflicts.length > 3) {
              message += `... and ${result.conflicts.length - 3} more\n`;
            }
          }
        ...
        autoAssignBtn.textContent = 'Auto-Assign Facilitators';
      }
    });
  }
}
