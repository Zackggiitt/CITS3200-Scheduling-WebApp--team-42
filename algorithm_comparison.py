...
class AdvancedSchedulingEngine:
    ...
    def _is_facilitator_available(self, facilitator: User, session: Session) -> bool:
        """Check if facilitator is available for the session (not unavailable)"""
        session_date = session.start_time.date()
        session_start_time = session.start_time.time()
        session_end_time = session.end_time.time()
        
        # Get all unavailability records for this facilitator on this date
        ...
        # If no unavailability records, facilitator is available
        ...
        for unavailability in unavailabilities:
            ...
            if (unavailability.start_time and unavailability.end_time and
                not (session_end_time <= unavailability.start_time or 
                     session_start_time >= unavailability.end_time)):
                return False
