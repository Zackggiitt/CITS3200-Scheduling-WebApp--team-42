...
def _get_user_unit_or_404(user, unit_id: int):
    """Return Unit if it exists AND is owned by user or user is facilitator; else None."""
    try:
        unit_id = int(unit_id)
    ...
    # Check if user is creator or facilitator
    if unit.created_by == user.id:
        return unit
    
    # Check if user is facilitator
    facilitator_link = UnitFacilitator.query.filter_by(unit_id=unit.id, user_id=user.id).first()
    if facilitator_link:
        return unit
...
def profile():
    ...
    # Get user's units for display (both created and as facilitator)
    units = (
        ...
        .outerjoin(UnitFacilitator, UnitFacilitator.unit_id == Unit.id)
        .filter(or_(Unit.created_by == user.id, UnitFacilitator.user_id == user.id))
        .group_by(Unit.id)
...
def auto_assign_facilitators(unit_id: int):
    """
    Auto-assign facilitators to sessions using the scheduling algorithm
    """
    user = get_current_user()
    unit = _get_user_unit_or_404(user, unit_id)
    if not unit:
    ...
    try:
        ...
        # Get sessions for this unit only
        sessions = (
        ...
        if not sessions:
            return jsonify({
                "ok": False, 
                "error": "No sessions found for this unit. Please create sessions first."
            }), 400
        ...
        assignments = []
        conflicts = []
        assigned_facilitators = set()
        facilitator_schedules = {}
        
        for session in sessions:
            best_facilitator = engine.find_best_facilitator_with_constraints(
                session, AlgorithmType.THRESHOLD_HYBRID, assigned_facilitators, facilitator_schedules
            )
            
            if best_facilitator:
                score = engine.calculate_score(best_facilitator, session, AlgorithmType.THRESHOLD_HYBRID)
                if score > 0:
                    # Create assignment in database
                    assignment = Assignment(
                        session_id=session.id,
                        facilitator_id=best_facilitator.id,
                        is_confirmed=False  # UC needs to confirm
                    )
                    ...
                    assignments.append({
                        'session_id': session.id,
                        'facilitator_id': best_facilitator.id,
                        'facilitator_name': f"{best_facilitator.first_name or ''} {best_facilitator.last_name or ''}".strip() or best_facilitator.email,
                        ...
                        'session_location': session.location or 'TBA',
                        'score': round(score, 2)
                    })
                    ...
                    conflicts.append({
                        ...
                        'session_info': f"{session.module.unit.unit_code} - {session.module.module_name}",
                        'session_time': session.start_time.strftime('%Y-%m-%d %H:%M'),
                        'reason': 'No suitable facilitator found (score too low)'
                    })
            else:
                conflicts.append({
                    'session_id': session.id,
        ...
        return jsonify({
            "ok": True,
            "message": f"Auto-assignment completed. {len(assignments)} sessions assigned, {len(conflicts)} conflicts.",
            "assignments": assignments,
            "conflicts": conflicts,
            "total_sessions": len(sessions),
            "assigned_sessions": len(assignments),
            "assignment_rate": round(len(assignments) / len(sessions) * 100, 1) if sessions else 0
        })
        
    except Exception as e:
        db.session.rollback()
        logger.error(f"Auto-assignment error: {str(e)}")
        return jsonify({"ok": False, "error": f"Auto-assignment failed: {str(e)}"}), 500
